...
/**
 * @param {Object} hook
 * @param {String} hook.cmd
 * @param {String} hook.cmd - cmd to be executed on remotehost
 ...
 */
async function executeRemoteHook (watcher, hook) {
  if (hook && typeof hook.output !== "string") {
    hook.output = "";
  }
  if (hook && hook.cmd) {
    hook.rt = await watcher.ssh.exec(hook.cmd, 10, (output) => { hook.output += output; });
    return true;
  }
  return false;
}
 ...
 * @return {Promise} - changed to return code after fullfilled
 */
async function executeLocalHook (hook) {
  const output = [];
  if (hook && hook.cmd) {
    hook.rt = await localExec(hook.cmd, output);
...
function hookHandler (watcher, remoteHook1, localHook1, remoteHook2, localHook2) {
  return Promise.all([
    executeRemoteHook(watcher, remoteHook1),
    executeLocalHook(localHook1),
    executeRemoteHook(watcher, remoteHook2),
    executeLocalHook(localHook2)
  ]);
}
  ...
  exec: async (watcher) => {
    ...
    for (const [key, request] of watcher.requests) {
      ...
      if (request.re && request.re.test(outputString) === Boolean(request.until)) {
        ...
        requestPromise.push(
          hookHandler(watcher, request.finishedHook, request.finishedLocalHook, request.doneHook, request.doneLocalHook)
            .then((e) => {
      ...
      request.maxCount < request.checkCount) {
        ...
        watcher.requests.delete(key);
        requestPromise.push(
          hookHandler(watcher, request.failedHook, request.failedLocalHook, request.doneHook, request.doneLocalHook)
            .then(() => {
