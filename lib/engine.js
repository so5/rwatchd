...
const debug = dbg("rwatchd:engine");
...
function isFinished (outputString, request) {
  if (!request.re) {
    debug("request does not have re");
    return false;
  }
  if (/^\s*$/.test(outputString)) {
    if (request.checkCount < request.numAllowFirstFewEmptyOutput) {
      debug(`empty output but first ${request.numAllowFirstFewEmptyOutput} times is allowed`);
      return false;
    }
    if (request.allowEmptyOutput) {
      debug("empty output but it is allowed");
      return false;
    }
  }
  return request.re.test(outputString) === Boolean(request.until);
}
...
const sbs = new SBS({
  ...
  exec: async (watcher) => {
    ...
    for (const [key, request] of watcher.requests) {
      ...
      if (isFinished(outputString, request)) {
