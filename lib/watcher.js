...
import Ajv from "ajv";
import SSH from "ssh-client-wrapper";
import { sbs } from "./engine.js";
import { addHistory } from "./history.js";
import { hostInfoSchema } from "./schema.js";

const debug = dbg("rwatchd:watcher");
 ...
 */
export function addWatcher (arg) {
 ...
 * @return {Watcher}
 */
export function getWatcherByRequestID (requestID) {
  for (const watcher of watchers.values()) {
...
export async function clearAllWatchers () {
  const p = [];
  for (const watcher of watchers.values()) {
